...
(deftest range-single-arity-test
  (testing "(range n) - loops from 0 to n-1"
    (let [result (cl/aloop (range 5) x [sum 0]
                   (if x
                     (recur (+ sum x))
                     sum))]
...
(deftest range-two-arity-test
  (testing "(range start end) - loops from start to end-1"
    (let [result (cl/aloop (range 3 7) x [sum 0]
                   (if x
                     (recur (+ sum x))
                     sum))]
...
(deftest range-three-arity-test
  (testing "(range start end step) - loops with custom step"
    (let [result (cl/aloop (range 0 10 2) x [sum 0]
                   (if x
                     (recur (+ sum x))
                     sum))]
...
(deftest range-length-test
  (testing "(length ::aloop) returns correct count for ranges"
    (testing "single arity"
      (let [len (cl/aloop (range 5) _ [l (cl/length ::aloop)]
                  (if _ (recur l) l))]
        (is (= 5 len) "length of (range 5) should be 5")))
    (testing "two arity"
      (let [len (cl/aloop (range 3 7) _ [l (cl/length ::aloop)]
                  (if _ (recur l) l))]
        (is (= 4 len) "length of (range 3 7) should be 4")))
    (testing "three arity with step"
      (let [len (cl/aloop (range 0 10 2) _ [l (cl/length ::aloop)]
                  (if _ (recur l) l))]
...
(deftest range-collect-elements-test
  (testing "collect all elements from range iteration"
    (let [result (cl/aloop (range 3 6) x [acc (cl/make-array 0)]
                   (if x
                     (do (cl/push-end acc x)
                         (recur acc))
                     acc))]
...
(deftest range-step-collect-test
  (testing "collect elements with step"
    (let [result (cl/aloop (range 1 10 3) x [acc (cl/make-array 0)]
                   (if x
                     (do (cl/push-end acc x)
                         (recur acc))
                     acc))]
