...
(deftest nested-aloop-length-scope-test
  (testing "each nested aloop has its own (length self) binding"
    ...
    (let [outer-collection [(cl/make-array 1 :initial-contents [11])
          ...
          ;; Outer aloop iterates over 2 rows; (cl/length self) here = 2
          inner-lengths (cl/aloop outer-collection
                                  [result (cl/make-array (cl/length self))  ; uses outer length (2)
                                   idx 0]
                                  (if it
                            ;; Capture outer 'it' before inner aloop shadows it
                                    (let [row it
                                  ;; Inner aloop iterates over each row's elements
                                  ;; (cl/length self) here should be row's length (1 or 3), NOT 2
                                          this-row-length (cl/aloop row [len (cl/length self)]
                                                                    (if it
                                                                      (recur len)
                                                                      len))]
                                    ...
                                    result))]
      ;; If scoping is broken, inner (cl/length self) would return 2 (outer's length)
